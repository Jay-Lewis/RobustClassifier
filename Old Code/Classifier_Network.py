...
class Classifier_simple(object):
    ...
    def __init__(self,x_size,y_size,model_directory):
        ...
        self.logits, self.class_vars = self.build_classifier(self.x)

        self.classifier_saver = tf.train.Saver(var_list=self.class_vars, max_to_keep=1)
        self.set_load_func()

    def build_classifier(x , reuse=False):
    ...
    def train_model(self, sess, x, y, train_epoch, train_op, num_epochs, batch_size):
    ...
    def train(self,sess,batch_size,num_epochs,learning_rate):
        ...
        train_epoch, _, test_epoch = utils.load_dataset(batch_size, self.load_func)
        print('training model')
        self.train_model(sess, self.x, self.y, train_epoch, train_op, num_epochs, batch_size)
        print('saving model')
